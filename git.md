...
## 与svn的区别
 - svn 管理的是文件的变动，就像是档案的仓库
 - git 存储的文件的快照，通过sha-1来区别不同的文件
 - svn 的核心是服务器
 - svn 是两层结构，本地和远程，本地需要提交到远程
 - git 是三层，工作区，暂存区，如果本地的提交没有推送到原服务器，不会影响到服务器上的代码
 - git 是权力下放的，所有的文件和日志都能在本地仓库进行查看和修改


## 为什么我们要切换到git
1. 节省时间

   - git是分布式的，所有的代码都是仓库的克隆，未修改之前与远程相同，大部分的操作都在本地进行，速度很快。
   - git保存的是快照，即每次的提交都会保存完整的文件，svn保存文件修改的方式在每次检出都要从开始的版本一个一个的修改叠加上去，很慢。
   - git因为是整个仓库的克隆，会在本地报错所有的log，可以在本地方便的查看log日志

2. 离线工作

   - git保存了完整的仓库信息，就算是没有网络的状态下，也可以在本地进行分支的管理和提交操作，只要在有网的时候再推送到远程就行了，这样就保证了我们在离线的时候工作的时候也能在开发新功能新分支的时候可以操作

3. 内容完整性 

   - git使用sha-1来唯一标识一个代码快照，降低对版本库的破坏

3. 回退错误

   - git log 保存了所有的版本信息，可以使用git reset HEAD （或者使用版本的id进行回退）
   - git reflog 保存了操作的日志，可以对操作进行回退

4. 可以只进行有用的提交

   - git 可以在本地任意的建立本地分支，在本地提交，如果不想推送到远程的话，他就一直是自己的本地分支
   - git 可以将多个提交汇合成一次提交推送到远程

5. 不会把东西搞混了

   - 你在某个新功能的分支上开发的时候，任何人都不会被你未完成的代码影响，也不会被任何人影响，你可以在本地专心的进行分支的开发

6. 潮流

   - 现在基本上所有的开源项目都在github上维护，git可以很方便的让所有的开发者都参与尽力啊，fork代码到你的仓库后，修改好代码之后只需要pull request，如果代码的管理者觉得你的代码可以，就可以向开源项目贡献代码了
